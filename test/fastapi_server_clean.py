...
# ONLY Real actions from Alpha Mini robot (user-provided list)
...
# ONLY Real expressions from Alpha Mini robot (user-provided list)
...
# Enhanced execution functions using real methods
...
async def execute_action(request: ActionRequest):
    ...
    try:
        # Use regular PlayAction API for all actions
        start_time = datetime.now()
        logger.info(f"⚡ Executing action: {request.name}")
        ...
        logger.info(f"📊 Action response type: {type(response)}, length: {len(response) if response else 0}")
        ...
        if response and len(response) > 1 and hasattr(response[1], 'isSuccess'):
            ...
            success = True  # Assume success if response format is unexpected
            message = f"Action '{request.name}' executed"
            logger.warning(f"⚠️ Unexpected response format for action {request.name}")
...
async def get_available_actions():
    """Get list of available actions - ONLY real ones"""
    logger.info(f"📋 Returning {len(REAL_ACTIONS)} real actions")
    return REAL_ACTIONS

@app.get("/api/expressions", response_model=List[ExpressionInfo])
async def get_available_expressions():
    """Get list of available expressions - ONLY real ones"""
    logger.info(f"📋 Returning {len(REAL_EXPRESSIONS)} real expressions")
    return REAL_EXPRESSIONS

@app.post("/api/actions/batch")
async def execute_action_batch(actions: List[str]):
    ...
    for action_name in actions:
        try:
            request = ActionRequest(name=action_name)
    ...
    return {"batch_results": results, "total_actions": len(actions)}
...
async def health_check():
    ...
    health_data = {
        "status": "healthy",
        "timestamp": datetime.now().isoformat(),
        "robot_connected": robot_state.is_connected,
        "total_actions_executed": robot_state.action_count,
        "connection_attempts": robot_state.connection_attempts,
        "last_error": robot_state.last_error,
        "uptime": "server_running",
        "available_actions": len(REAL_ACTIONS),
        "available_expressions": len(REAL_EXPRESSIONS)
    }
...
async def startup_event():
    ...
    logger.info(f"🎭 Available actions: {len(REAL_ACTIONS)}")
    logger.info(f"😊 Available expressions: {len(REAL_EXPRESSIONS)}")
...
if __name__ == "__main__":
    ...
    print(f"🎭 Real actions available: {len(REAL_ACTIONS)}")
    print(f"😊 Real expressions available: {len(REAL_EXPRESSIONS)}")
